...
class Covpool(Function):
     ...
     def forward(ctx, input):
         ...
         # torch.eye 创建一个单位矩阵，对角线为1
         I_hat = (-1./M/M)*torch.ones(M,M,device = x.device) + (1./M)*torch.eye(M,M,device = x.device)
         # tensor.repeat(batchsize,1,1)，就是对应每个维度上的数都复制相应的倍数
         I_hat = I_hat.view(1,M,M).repeat(batchSize,1,1).type(x.dtype)
         # x.bmm(I_hat)求的是矩阵的值减去他每个维度上的均值
         # 后来乘以x的转置，与协方差的定义不一致，最终得到的是dim*dim的协方差矩阵
         y = x.bmm(I_hat).bmm(x.transpose(1,2))# tensor.bmm就是矩阵相乘，只是这边要求两个值都要是3D的
...
class Sqrtm(Function):
     ...
     def forward(ctx, input, iterN):
         ...
         A = x.div(normA.view(batchSize,1,1).expand_as(x))# div是做除法
